...
def format_stats(options, process, duration):
    ...
    def count2qps(count):
        return '%.2f/s ' % (count / (duration * 1.0))
    ...
    read_char = display_format(stats.read_char, duration)
    write_char = display_format(stats.write_char, duration)
    blkio_count = count2qps(stats.blkio_count)
    swapin_count = count2qps(stats.swapin_count)
    read_syscalls = count2qps(stats.read_syscalls)
    write_syscalls = count2qps(stats.write_syscalls)

    return io_delay, swapin_delay, read_bytes, write_bytes, read_char, write_char, blkio_count, swapin_count, read_syscalls, write_syscalls
...
class IOTopUI(object):
    ...
    sorting_keys = [
        ...
        (lambda p, s: s.read_char, True),
        (lambda p, s: s.write_char, True),
        (lambda p, s: s.read_syscalls, True),
        (lambda p, s: s.write_syscalls, True),
        (lambda p, s: s.blkio_count, True),
        (lambda p, s: s.swapin_count, True),
    ...
    def __init__(self, win, process_list, options):
        ...
        self.__set_sorting_key(options)
    ...
    def __set_sorting_key(self, options):
        # default
        if not options.sort_key:
            self.sorting_key = 12
            return
        # DISK_READ
        if options.sort_key == 'DISK_READ':
            self.sorting_key = 3
        # DISK_WRITE
        elif options.sort_key == 'DISK_WRITE':
            self.sorting_key = 4
        # read_char
        elif options.sort_key == 'BUFFER_READ':
            self.sorting_key = 5
        # write_char
        elif options.sort_key == 'BUFFER_WRITE':
            self.sorting_key = 6
        # read syscall
        elif options.sort_key == 'READ_SYSCALLS':
            self.sorting_key = 7
        # write syscall
        elif options.sort_key == 'WRITE_SYSCALLS':
            self.sorting_key = 8
        # blkio_count
        elif options.sort_key == 'BLKIO_CNT':
            self.sorting_key = 9
        # swapin_count
        elif options.sort_key == 'SWAPIN_CNT':
            self.sorting_key = 10
        # SWAPIN
        elif options.sort_key == 'SWAPIN':
            self.sorting_key = 11
        else:
            self.sorting_key = 12
        return
    ...
    def run(self):
        ...
        while self.options.iterations is None or \
            ...
            self.refresh_display(iterations == 0, total, actual,
                                 self.process_list.duration, self.options.snap)
            if self.options.iterations is not None:
                ...
                iterations = 1
            # not for snapshot use, poll for keyboard event
            if not self.options.snap:
                ...
                if events:
                    key = self.win.getch()
                    self.handle_key(key)
            # for snapshot use, sleep for delay_seconds
            else:
    ...
    def get_data(self):
        def format(p):
            ...
            io_delay, swapin_delay, read_bytes, write_bytes, read_char, write_char, blkio_count, swapin_count, read_syscalls, write_syscalls = stats
            if Stats.has_blkio_delay_total:
                delay_stats = '%7s %7s ' % (swapin_delay, io_delay)
            else:
                delay_stats = ' ?unavailable?  '
            pid_format = '%%%dd' % MAX_PID_WIDTH
            line = (pid_format + ' %4s %-8s %11s %11s %12s %12s %14s %14s %12s %12s %s') % (
                p.pid, p.get_ioprio(), p.get_user()[:8], read_bytes,
                write_bytes, read_char, write_char, read_syscalls, write_syscalls, blkio_count, swapin_count, delay_stats)
    ...
    def refresh_display(self, first_time, total, actual, duration, snap):
        # if for os snap-shot, ignore first output
        ...
        if self.options.processes:
            ...
            pid += 'TID'
        titles = [pid, '  PRIO', '  USER', '     DISK READ', '  DISK WRITE', '  BUFFER READ', ' BUFFER WRITE',
                  ' READ SYSCALLS', ' WRITE SYSCALLS',
                  '    BLKIO CNT', '   SWAPIN CNT',
                  '   SWAPIN', '      IO', '    COMMAND']
        lines = self.get_data()
...
def main():
    ...
    parser.add_option('-s', '--sort_key', type='str', action='store', dest='sort_key',
                      default='BLKIO', help='specify sort key, possible values include DISK_READ, DISK_WRITE, BUFFER_READ, BUFFER_WRITE, READ_SYSCALLS, WRITE_SYSCALLS,'
                                         'BLIKIO_COUNT, SWAPIN_COUNT, SWAPIN, BLKIO')
    parser.add_option('--snap', action='store_true', help='for use of os-snap, ignore first output and will not poll for keyboard interrupts', dest='snap', default=False)
